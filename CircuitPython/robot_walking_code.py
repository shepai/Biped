...
for z in range(10): #loop through the sample size of rates (10%, 20%,..., N%)
    ...
    lastFitness=0
    store=[]
    genotype=Mgenotype.copy() #copy over the main genotype to give a fair experiment
    for i in range(15): #total of 15 generations
        sleep(1) #time to reset if fallen over
        distNo=getDist() #start dist
        print("Generation",i+1)
        currentGeno=mutate(genotype.copy(),rate=((z+1)/10)) #mutate via rate defined by z
        ...
        sleep(1)
    print(genotype)
    print(j,store) #show console what is going on
